...
import { useCart, CartRestourant } from "@/app/(order)/hooks/useCart";
...
export default function PaymentPage() {
  ...
  // Ambil data restoran dan keranjang menggunakan hook
  const { restaurant, loading: restaurantLoading } = useRestaurant(restaurantId!);
  const { cart, calculateCartTotals } = useCart(restaurant);
  ...
  // Ambil ID restoran dari keranjang di localStorage saat komponen dimuat
  useEffect(() => {
    const savedCartString = localStorage.getItem("otter-cart");
    if (savedCartString) {
      const savedCart: CartRestourant[] = JSON.parse(savedCartString);
      if (savedCart.length > 0) {
        setRestaurantId(savedCart[0].$id); // Asumsikan hanya ada 1 restoran di keranjang
      } else {
  ...
  // Helper untuk menghitung total per item (termasuk opsi)
  const calculateItemTotal = (item: any) => {
    const itemPrice = item.discountPrice ?? item.price;
    const optionsPrice = Object.values(item.selectedOptions).reduce(
      (sum: number, options: any) =>
        sum +
        options.reduce(
          (optSum: number, opt: any) => optSum + (opt.discountPrice ?? opt.price),
          0
        ),
      0
    );
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-6">
      <div className="max-w-md mx-auto bg-white shadow-sm">
          ...
          <>
            ...
            <div className="px-4 py-5">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Service Fee ({totals.servicePercentage}%)</span>
            ...
            {/* Customer Details & Payment Method (Sama seperti sebelumnya) */}
        ...
        {currentStep === 2 && (
          <div className="px-4 py-5">
             ...
             <Button variant="outline" className="w-full py-6 flex items-center justify-center gap-2 border-dashed border-2" onClick={handleViewReceipt}><FileText className="h-5 w-5" /><span className="font-medium">Click here for your e-receipt</span></Button>
          </div>
        )}

        {/* Footer */}
        <div className="px-4 py-5 bg-white border-t sticky bottom-0">
          {currentStep === 0 && (<Button className="w-full h-12 bg-black hover:bg-black/90" onClick={handleContinue} disabled={!name || !isPhoneValid}>Continue to Payment</Button>)}
      ...
      <PaymentMethodDrawer isOpen={isMethodDrawerOpen} onOpenChange={setIsMethodDrawerOpen} onSelectMethod={handleSelectPaymentMethod} />
    </div>
  );
}