...
import { useRouter, useSearchParams } from "next/navigation"; // Gunakan dari next/navigation
...
export default function PaymentPage() {
  ...
  // State untuk menampung data dari URL
  ...
  // Ambil dan proses data dari URL saat komponen dimuat
  useEffect(() => {
    ...
    if (dataString) {
      try {
        ...
        if (!parsedData.cart || parsedData.cart.length === 0) {
            router.replace('/'); // Kembali jika keranjang kosong
        }
      } catch (error) {
        console.error("Failed to parse cart data from URL", error);
        router.replace('/'); // Kembali jika data tidak valid
      }
    } else {
      router.replace('/'); // Kembali jika tidak ada data
    }
  ...
  // Helper untuk menghitung total per item (termasuk opsi) untuk TAMPILAN
  ...
  const handleContinue = () => {
    if (currentStep === 0) {
      ...
      if (!isPhoneValid) { toast({ title: "Valid phone number is required", variant: "destructive" }); return; }
      setCurrentStep(1);
    }
  };
  ...
  // Tampilkan skeleton saat data belum siap
  if (!totals) {
    return (
      ...
      <div className="max-w-md mx-auto bg-white shadow-sm">
        ...
        {currentStep === 0 && (
          <>
            ...
            {/* Customer Details & Payment (tidak berubah) */}
