...
import { menuItems } from "@/app/(online-order)/restaurant/sample-data";
...
export function getRecommendations(cart: CartItem[], limit = 3): MenuItem[] {
  ...
  if (cartCategories.has("Milk Tea Series")) {
    ...
    recommendations.push(
      ...allItems.filter(
        (item) => item.category === "Toppings" && !cartItemIds.has(item.id)
      )
    );
  }
  ...
  if (cartCategories.has("Classic Tea Series")) {
    ...
    recommendations.push(
      ...allItems.filter(
        (item) =>
          item.category === "Milk Tea Series" && !cartItemIds.has(item.id)
      )
    );
  }
  ...
  if (cartCategories.has("Fruit Tea Series")) {
    ...
    recommendations.push(
      ...allItems.filter(
        (item) =>
          item.category === "Fruit Tea Series" && !cartItemIds.has(item.id)
      )
    );
  }
