      ...
      <div className="relative w-32 h-32 flex-shrink-0">
        {item.image && !imageError ? (
          <div className={`w-full h-full rounded-lg overflow-hidden ${
            itemInStock ? "bg-muted" : "bg-muted/50"
          }`}>
            <Image
              ...
              className={`object-cover rounded-lg ${
                itemInStock ? "" : "grayscale"
              }`}
              onError={() => setImageError(true)}
            />
          </div>
        ) : (
          <div className={`w-full h-full rounded-lg overflow-hidden border-2 flex items-center justify-center p-2 text-center ${
            itemInStock 
              ? "bg-yellow-50 border-yellow-100" 
              : "bg-gray-50 border-gray-100"
          }`}>
            <span className={`font-medium text-sm whitespace-pre-line ${
              itemInStock ? "text-yellow-800" : "text-gray-500"
            }`}>
        ...
        <div className="flex justify-between items-start gap-3">
          <div className="space-y-1 flex-1 min-w-0">
            ...
            <p className={`text-sm line-clamp-2 ${
              itemInStock ? "text-muted-foreground" : "text-gray-400"
            }`}>
              {item.description}
            </p>
